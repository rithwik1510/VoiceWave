...
fn main() {
    ...
    if cfg!(target_os = "windows") {
        // /utf-8 is a MSVC-only flag and breaks MinGW GNU toolchains.
        if !target.contains("gnu") {
    ...
    let link_roots = vec![
        ...
        out.join("lib"),
        out.join("build"),
        out.join("build").join("src"),
        out.join("build").join("ggml"),
        out.join("build").join("ggml").join("src"),
    ];

    for root in &link_roots {
        add_link_search_path(root).unwrap();
    }

    if !emit_link_if_present("whisper", cfg!(feature = "intel-sycl"), &link_roots) {
        panic!("whisper-rs-sys could not locate built whisper static library");
    }
    emit_link_if_present("ggml", cfg!(feature = "intel-sycl"), &link_roots);
    emit_link_if_present("ggml-base", cfg!(feature = "intel-sycl"), &link_roots);
    emit_link_if_present("ggml-cpu", cfg!(feature = "intel-sycl"), &link_roots);

    if cfg!(target_os = "macos") || cfg!(feature = "openblas") {
        emit_link_if_present("ggml-blas", false, &link_roots);
    }
    if cfg!(feature = "vulkan") {
        emit_link_if_present("ggml-vulkan", cfg!(feature = "intel-sycl"), &link_roots);
    }

    if cfg!(feature = "hipblas") {
        emit_link_if_present("ggml-hip", false, &link_roots);
    }

    if cfg!(feature = "metal") {
        emit_link_if_present("ggml-metal", false, &link_roots);
    }

    if cfg!(feature = "cuda") {
        emit_link_if_present("ggml-cuda", false, &link_roots);
    }
    ...
    if cfg!(feature = "intel-sycl") {
        emit_link_if_present("ggml-sycl", true, &link_roots);
    }
...
fn emit_link_if_present(lib_name: &str, dynamic: bool, roots: &[PathBuf]) -> bool {
    let found = find_library_file(lib_name, roots);
    if let Some(lib_file) = found {
        if dynamic {
            println!("cargo:rustc-link-lib={}", lib_name);
        } else {
            let file_name = lib_file
                .file_name()
                .map(|value| value.to_string_lossy().to_string())
                .unwrap_or_else(|| format!("lib{}.a", lib_name));
...
fn find_library_file(lib_name: &str, roots: &[PathBuf]) -> Option<PathBuf> {
    let candidates = [
